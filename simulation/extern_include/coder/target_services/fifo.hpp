...
struct circular_iterator : public std::iterator<std::random_access_iterator_tag, value_type>
{
    ...
    circular_iterator<value_type>  operator++(int) { coder::tgtsvc::detail::circular_iterator<value_type> temp(*this); advance(); return temp; }
    circular_iterator<value_type>  operator--(int) { coder::tgtsvc::detail::circular_iterator<value_type> temp(*this); retreat(); return temp; }
    circular_iterator<value_type> operator+(ptrdiff_t dist) const { coder::tgtsvc::detail::circular_iterator<value_type> temp(*this); temp.advance(dist); return temp; }
    circular_iterator<value_type> operator-(ptrdiff_t dist) const { coder::tgtsvc::detail::circular_iterator<value_type> temp(*this); temp.advance(-dist); return temp; }
