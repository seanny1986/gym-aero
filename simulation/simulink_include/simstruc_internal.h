...
#define _ssGetLocalErrorStatus(S) \
       ...
       NULL : S->blkInfo.blkInfo2->localErrorStatus.str )
...
#define _ssGet_slLocalErrMsg(S) \
       ...
       NULL : S->blkInfo.blkInfo2->localErrorStatus.msg )
...
#define _ssIsErrorStatusAslErrMsg(S) \
        ( ssGetRootSS(S)->mdlInfo->mdlFlags.errorStatusIsMsg == 1U )
#define _ssIsLocalErrorStatusAslErrMsg(S) \
        ( (S)->blkInfo.sfcnFlags.localErrorStatusIsMsg == 1U )
...
#define _ssCopyLocalErrorStatusToBuffer(S, string, size) \
      { (void)memcpy((S)->blkInfo.blkInfo2->localErrorStatusBuffer, (string), ((size) > SS_ERROR_STATUS_BUFFER_SIZE-1)? (SS_ERROR_STATUS_BUFFER_SIZE-1) : (size) ); \
...
#define ssSetBusPropagationSupported(S, value)    \
    ssSetBusPropagationSupported_cannot_be_used_in_RTW

#define ssSetBusSourcePort(S, value)            \
    ssSetBusSourcePort_cannot_be_used_in_RTW

#define ssSetBusInputPorts(S, numInports, value) \
  ssSetBusInputPorts_cannot_be_used_in_RTW
 ...
 */
# define _ssSetOwnerCbd(S,cbd) \
    (S)->work.localMdlInfo->ownerCbd = ((void *) cbd)
# define _ssGetOwnerCbd(S) \
          (S)->work.localMdlInfo->ownerCbd                /*   (void*)    */

#if !SS_SFCN
# define ssSetOwnerCbd(S,cbd) _ssSetOwnerCbd(S,cbd)
#else
# define ssSetOwnerCbd(S,cbd) \
            ssSetOwnerCbd_cannot_be_used_in_SFunctions
#endif

#if !SS_SFCN || SS_GENERATED_S_FUNCTION
# define ssGetOwnerCbd(S) _ssGetOwnerCbd(S)
#else
#define ssGetOwnerCbd(S) \
            ssGetOwnerCbd_cannot_be_used_in_SFunctions
#endif
 ...
 */
# define _ssSetOwnerEbd(S,ebd) \
    (S)->work.localMdlInfo->ownerEbd = ((void *) ebd)
# define _ssGetOwnerEbd(S) \
          (S)->work.localMdlInfo->ownerEbd                /*   (void*)    */
...
#else
#define ssGetOwnerEbd(S) \
            ssGetOwnerEbd_cannot_be_used_in_SFunctions
#endif
...
#define ssGetSolverOrder(S) \
          ssGetSolverInfo(S)->solverOrder                    /*   (int_T)       */
#define _ssSetSolverOrder(S,order) \
          ssGetSolverInfo(S)->solverOrder = (order)
#if !SS_SFCN
#define ssSetSolverOrder(S,order) _ssSetSolverOrder(S,order)
#else
#define ssSetSolverOrder(S,order) \
          ssSetSolverOrder_cannot_be_used_in_SFunctions
#endif
