...
class PlannerPEBEnv(gym.Env):
    def __init__(self, max_rad=5., length=5, width=5, height=5):
        ...
        self.action_space2 = np.zeros((15,))
    ...
    def get_reward(self, next_state):
        ...
        if np.linalg.norm(vec_xyz) < self.goal_thresh:
            cmplt_rew = 100.*(self.steps-self.count)
    ...
    def step(self, action):
        ...
        next_state = next_state.T.tolist()[0]+self.vec_xyz.T.tolist()[0]+[(self.steps-self.count)/self.steps]
        lim = False
        if self.count == self.steps:
            lim = True
        self.count += 1
        return next_state, rew, done, True, {"dist_rew": info[0],
                                            "guide_rew": info[1],
                                            "cmplt_rew": info[2],
                                            "step_rew": info[3],
                                            "oob_rew": info[4]}
    ...
    def reset(self):
        ...
        self.xyz = xyz
        return state+self.vec_xyz+[(self.steps-self.count)/self.steps]
    ...
    def render(self, mode='human', close=False):
        ...
        self.ani.draw_label("Time: {}".format(self.count), 
