...
class TrajectoryEnvTermCircle(gym.Env):
    """
    ...
    def __init__(self):
        ...
        self.zeta_list = []
        x = np.zeros((3,1))
        self.dr = pi/self.traj_len
        self.radius = self.r_max/2/sin(self.dr/2)
        self.angles = np.arange(self.dr, pi+self.dr, self.dr)
        for i in range(self.traj_len):
            new_waypoint = x+self.generate_goal(self.angles[i])
    ...
    def generate_goal(self, angle):
        ...
        y = self.radius*cos(angle)-self.radius
        z = 0.
