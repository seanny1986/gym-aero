...
class TrajectoryEnvTermFut(gym.Env):
    """
    ...
    def __init__(self):
        ...
        self.zeta_list = []
        x = np.zeros((3,1))
        new_waypoint = np.array([[1.],[0.],[0.]])
        self.goal_list.append(new_waypoint)
        sin_zeta, cos_zeta = np.sin(np.array([[0.],[0.],[pi/4]])), np.cos(np.array([[0.],[0.],[pi/4]]))
        self.zeta_list.append([sin_zeta, cos_zeta])
        x = new_waypoint
        for i in range(1):
            new_waypoint, self.zeta_list = self.generate_goal()
            self.goal_list.append(x+new_waypoint)         
        self.goal = 0
    ...
    def reward(self, state, action):
        ...
        ctrl_rew -= 75.*np.sum((uvw-self.prev_uvw)**2)
    ...
    def generate_goal(self):
        ...
        angle = np.random.uniform(low=0., high=pi)
        x = self.radius*sin(angle)
        y = self.radius*cos(angle)
        z = 0.
        angle = np.array([[0.],[0.],[angle]])
        return np.array([[x],[y],[z]]), [[np.sin(angle/2), np.cos(angle/2)], [np.sin(angle), np.cos(angle)]]
