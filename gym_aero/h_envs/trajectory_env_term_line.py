...
class TrajectoryEnvTermLine(gym.Env):
    """
    ...
    def reward(self, state, action):
        ...
        lin_accel = -50.*np.sum((uvw-self.prev_uvw)**2)
        ang_accel = -50.*np.sum((pqr-self.prev_pqr)**2)

        # agent gets a slight negative reward for time spent in flight
        time_rew = 0.
        return dist_rew, att_rew, vel_rew, ang_rew, ctrl_rew, lin_accel, ang_accel, time_rew
