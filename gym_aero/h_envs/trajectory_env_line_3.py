...
class TrajectoryEnvTermLineThree(gym.Env):
    """
    ...
    def __init__(self):
        ...
        self.observation_space = np.zeros((65,))
        self.bandwidth = 35.
        self.init_rendering = False
    
    def get_distance(self):
        return np.linalg.norm(self.vec_xyz)
    ...
    def reward(self, state, action):
        ...
        uvw_accel_rew = -75.*np.sum((uvw-self.prev_uvw)**2)
        pqr_accel_rew = -50.*np.sum((pqr-self.prev_pqr)**2)

        # agent gets a slight negative reward for time spent in flight
        time_rew = 0.
        return dist_rew, att_rew, vel_rew, ang_rew, ctrl_rew, time_rew, uvw_accel_rew, pqr_accel_rew
    ...
    def next_goal(self, state):
        ...
        if self.goal_next_next >= len(self.goal_list)-1:
            self.goal_xyz_next_next = np.zeros((3,1))
            self.goal_zeta_sin_next_next = np.zeros((3,1))
            self.goal_zeta_cos_next_next = np.zeros((3,1))
        else:
            self.goal_next_next += 1
            self.goal_xyz_next_next = self.goal_list[self.goal_next_next]
            self.goal_zeta_sin_next_next = self.zeta_list[self.goal_next_next][0]
            self.goal_zeta_cos_next_next = self.zeta_list[self.goal_next_next][1]
    ...
    def step(self, action, term):
        ...
        next_attitude = s_zeta.T.tolist()[0]+c_zeta.T.tolist()[0]
        ...
        # add goals to next to state observation
        next_next_position_goal = (xyz-self.goal_xyz_next_next).T.tolist()[0]
        next_next_attitude_goal = (s_zeta-self.goal_zeta_sin_next_next).T.tolist()[0]+(c_zeta-self.goal_zeta_cos_next_next).T.tolist()[0]
        next_next_velocity_goal = self.vec_uvw.T.tolist()[0]+self.vec_pqr.T.tolist()[0]
        next_next_goals = next_next_position_goal+next_next_attitude_goal+next_next_velocity_goal

        next_state = next_state+current_rpm+goals+next_goals+next_next_goals+[self.time_state]
    ...
    def reset(self):
        ...
        self.goal_next_next = self.goal_next+1
        ...
        state = state+current_rpm+goals+next_goals+next_next_goals+[self.time_state]
        return state
    ...
    def generate_goal(self):
        ...
        goal_xyz = np.array([[1.],[0.],[0.]])
        mag = np.linalg.norm(goal_xyz)
        return (goal_xyz/mag)*self.r_max
